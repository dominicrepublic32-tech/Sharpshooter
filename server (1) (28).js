...
class Account {
  ...
  openClosedCandleStream(symbol, cb, granularity = 60){
    const ws = new WebSocket(WS_DEMO);
    let lastOpenTime = null; // tracks open_time of the currently forming candle
    let lastOhlc     = null; // last ohlc received (final values of forming candle)
    ws.on('open', () => ws.send(JSON.stringify({ ticks_history: symbol, adjust_start_time: 1, count: 50, end: 'latest', granularity, style: 'candles', subscribe: 1 })));
    ws.on('message', raw => {
      let m; try { m = JSON.parse(raw); } catch { return; }

      // Historical closed candles snapshot — feed directly, seed lastOpenTime
      if (m.msg_type === 'candles' && m.candles){
        m.candles.forEach(c => cb(c, false));
        // Seed lastOpenTime from the last historical candle's epoch
        // so the first live ohlc doesn't false-trigger a close
        if (m.candles.length > 0){
          lastOpenTime = String(m.candles[m.candles.length - 1].epoch);
        }
        return;
      }

      // Live ohlc — fires on every tick for the currently forming candle
      // open_time stays the same while the candle forms
      // When open_time changes, the previous candle just closed
      if (m.msg_type === 'ohlc' && m.ohlc){
        const ohlc     = m.ohlc;
        const openTime = String(ohlc.open_time);

        if (lastOpenTime !== null && openTime !== lastOpenTime && lastOhlc){
          // open_time changed — lastOhlc is the fully closed candle with final OHLC values
          cb(lastOhlc, true);
        }
