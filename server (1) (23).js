...
class Account {
  ...
  openClosedCandleStream(symbol, cb, granularity = 60){
    ...
    ws.on('message', raw => {
      let m; try { m = JSON.parse(raw); } catch { return; }
      // Historical + newly closed candles — the API adds the just-closed candle
      // to this list the moment a new one opens. Only process ones we haven't seen.
      if (m.msg_type === 'candles' && m.candles){
        m.candles.forEach(c => {
          if (!seen.has(c.epoch)){
            seen.add(c.epoch);
            cb(c, false); // false = historical/closed
          }
        });
        return;
      }
      // ohlc = live forming candle tick — NEVER passed to Super Logic
      // But when a new ohlc epoch arrives, it means a new candle just opened,
      // so the API will send an updated candles list shortly containing the sealed candle.
      // We do nothing here — the candles message handles everything.
    });
    ws.on('error', () => {}); return ws;
  }
...
function superOnCandle(slot, candle){
  ...
  if (slot.superState === 'triggering'){
    ...
    if (color !== oppositeColor){
      // Original color returned before trigger is complete — entire setup invalid
      slot.emit(`Super: trigger broken by ${color} candle — discarding setup, starting fresh`);
      // This candle becomes the new run start
      superReset(slot);
      slot.superRun      = [candle];
      slot.superRunColor = color === 'doji' ? null : color;
      if (color === 'doji') slot.superRun = [];
      return;
    }
...
function startWatcher(slot){
  ...
  if (cfg.logic === 'super'){
    let m1Ready = false; // true after historical seed is done
    slot.dataWs = acct.openClosedCandleStream(cfg.symbol, (candle, isNew) => {
      if (!slot.running) return;
      // Always feed into M1 trend buffer
      slot.trendBuf.push(candle); if (slot.trendBuf.length > 100) slot.trendBuf.shift();
      // First batch = historical seed — don't dispatch as signals
      if (!m1Ready){ return; }
      // This is a newly closed candle the bot hasn't seen before — dispatch
      dispatch(slot, candle);
    }, 60);
    // Mark historical seed complete after a short delay (history arrives in one batch)
    setTimeout(() => { m1Ready = true; }, 2000);

    // M5 stream — closed candles for trend detection only
    ...
    // M15 stream — closed candles for trend detection only (m1_5_15 mode)
    if (cfg.super_trend_mode !== 'm1_5'){
