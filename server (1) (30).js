...
class Account {
  ...
  openSuperStream(symbol, onM1Closed, onM5Candle, onM15Candle, trendMode){
    const ws = new WebSocket(WS_DEMO);
    // Map req_id → granularity so we can route candles snapshots correctly
    const reqMap = { 1: 60, 2: 300, 3: 900 };
    // Per-granularity state for closed candle detection
    const state = {};
    ...
    ws.on('message', raw => {
      ...
      // Historical snapshot — use req_id to identify granularity
      if (m.msg_type === 'candles' && m.candles && m.candles.length > 0){
        ...
        // Seed lastOpenTime so first live ohlc doesn't false-trigger
        state[g].lastOpenTime = String(m.candles[m.candles.length - 1].epoch);
        // Feed history to trend buffers (M5/M15 only)
        if (g === 300) m.candles.forEach(c => onM5Candle(c));
        if (g === 900) m.candles.forEach(c => onM15Candle(c));
        return;
      }

      // Live ohlc — granularity field IS inside ohlc object (confirmed from debug)
      if (m.msg_type === 'ohlc' && m.ohlc){
        ...
        if (s.lastOpenTime !== null && openTime !== s.lastOpenTime && s.lastOhlc){
          // open_time changed — s.lastOhlc is the sealed closed candle
