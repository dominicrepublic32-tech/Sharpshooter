...
async function findRotativeBarrier(symbol, sign, stake, duration, durationUnit, targetPayout, maxIterations, contractType, tolerancePct){
  ...
  let lo = 0.01, hi = 1, loRes, hiRes;
  try { loRes = await priceAt(lo); } catch { return null; } // can't even price the smallest possible offset — genuine failure
  track(loRes);
  if (withinBand(loRes.payout)) return loRes;
  if (loRes.payout >= targetPayout) return bestRes; // target is at/below the smallest offset's payout and outside tolerance — fire with the smallest offset anyway rather than skip

  let iterations = 1;
  try { hiRes = await priceAt(hi); iterations++; }
  catch { return bestRes; } // couldn't price the initial high bound — fall back to best so far
  track(hiRes);
  if (withinBand(hiRes.payout)) return hiRes;
  let expandAttempts = 0;
  while (!isNaN(hiRes.payout) && hiRes.payout < targetPayout && expandAttempts < 15 && iterations < maxIterations){
    hi *= 2;
...
async function resolveTradeBarrier(slot, sign, contractType){
  ...
  const result = await findRotativeBarrier(c.symbol, sign, c.stake, c.duration_value, c.duration_unit, targetPayout, maxIter, contractType, tolerancePct)
    .catch(() => null);
  if (!result) {
    slot.emit(`Rotative Barrier: couldn't price this trade at all (no live quote available) — skipping`, 'err');
    return null;
  }
