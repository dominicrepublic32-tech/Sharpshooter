 ...
 *
 * PAGINATION — backward via `end`+`count`, confirmed against the real
 * Deriv API with a diagnostic script (see diagnose_deriv.js): `start` is
 * always ignored; the server always returns the most recent up-to-`count`
 * ticks ending at `end`. To walk backward through history: request a page
 * ending "now", note the OLDEST epoch received, set the next request's
 * `end` to (that epoch - 1), and repeat.
 *
...
class HistoryDownloader {
  ...
  async _runSegment(job, segStart, segEnd, onProgress) {
    ...
    while (job.cursor >= segStart) {
      ...
      try {
        page = await this._fetchPage(job.symbol, requestedEnd);
      } catch (err) {
        job.status = 'error';
        job.error = err.message;
  ...
  /** Backward-pagination request: `end` + `count`, no `start` (see file header). */
  async _fetchPage(symbol, end) {
    const resp = await this._send({ ticks_history: symbol, end, count: this.pageSize, style: 'ticks' });
    if (!resp.history) {
...
const dataCandleBuilder = new CandleBuilder(dataDb, DATAENGINE_CANDLE_INTERVAL_SECONDS, { info: serverLog, warn: serverLog, error: serverLog });
const historyDownloader = new HistoryDownloader(dataDb, dataCandleBuilder, WS_DEMO, { info: serverLog, warn: serverLog, error: serverLog });
const liveTickCollector = new LiveTickCollector(dataDb, dataCandleBuilder, candleManager, { info: serverLog, warn: serverLog, error: serverLog });
...
function resolveContractFromHistory(ticks, startIndex, params) {
  ...
  if (params.duration_unit === 's') expireEpoch = entryTick.epoch + params.duration;
  if (params.duration_unit === 'm') expireEpoch = entryTick.epoch + params.duration * 60;
...
class BacktestRunner {
  ...
  async run() {
    try {
      ...
      const needsCandles = this.slot.cfg.observe_mode === 'candles' || this.slot.cfg.logic === 'super';
      const candleQueue = needsCandles ? dataDb.getCandlesBetween(this.symbol, this.fromEpoch, this.toEpoch) : [];
      let candleIdx = 0;
      ...
      for (let i = 0; i < this.allTicks.length; i++) {
        ...
        if (needsCandles) {
          ...
          while (candleIdx < candleQueue.length && candleQueue[candleIdx].epoch + 60 <= tick.epoch) {
            dispatch(this.slot, candleQueue[candleIdx]);
            candleIdx++;
          }
        } else {
...
function goldenTrendConfirmed(slot, side){
  ...
  const m1Trend  = detectTrend(slot.trendBuf,    Number(cfg.golden_trend_lookback_m1)  || 10);
  const m5Trend  = detectTrend(slot.superM5Buf,  Number(cfg.golden_trend_lookback_m5)  || 8);
  const m15Trend = detectTrend(slot.superM15Buf, Number(cfg.golden_trend_lookback_m15) || 6);
...
app.get('/callback', async (req, res) => {
  ...
  try {
    ...
    // FIX: previously this auto-picked accounts[0] and connected the bot to
    // whichever account happened to be first in Deriv's response — which is
    // NOT necessarily the account the user has actually funded. Now we stop
    // here, hand the full list to the frontend, and wait for the user to
    // explicitly pick which account to trade on (see /api/oauth/select-account).
    pendingLiveAuth = { accessToken, accounts, expires: Date.now() + 10 * 60 * 1000 };
    push({
      type: 'live_accounts',
      accounts: accounts.map(a => ({
        account_id: a.account_id, currency: a.currency, balance: a.balance,
        account_type: a.account_type || a.type || null, landing_company: a.landing_company_name || null,
      })),
    });
    push({ type:'live_status', status:'working', msg:`Found ${accounts.length} account(s) — choose one to connect` });
  } catch (err) {
