...
function defaultCfg(){
  return {
    ...
    // Two INDEPENDENTLY toggleable ways to ease strictness back down (tightening
    // on a loss always happens regardless of these — only easing is optional):
    // 1) win-streak easing (the original mechanism) — on by default, preserves
    //    existing behavior for anyone already using automatic mode.
    // 2) price-movement easing (added) — off by default (opt-in): if price
    //    drifts golden_trend_auto_price_ease_points in EITHER direction from
    //    wherever the level last changed, ease down one step even with no
    //    trade having fired at all — prevents getting stuck at max strictness
    //    through a long quiet stretch just because nothing fired to prove a
    //    win. Each ease resets the reference price, so the next threshold is
    //    measured fresh from the new level (stair-steps down on sustained
    //    movement, e.g. 10pts: 80→70, another 10pts: 70→60).
    golden_trend_auto_win_ease_enabled:true,
    golden_trend_auto_price_ease_enabled:false, golden_trend_auto_price_ease_points:10,
    ...
    // Same two independently-toggleable easing mechanisms as Golden Logic
    // above — kept fully separate per-logic per existing convention.
    super_trend_auto_win_ease_enabled:true,
    super_trend_auto_price_ease_enabled:false, super_trend_auto_price_ease_points:10,
...
class Slot {
  constructor(idx){
    ...
    this.goldenTrendAutoLevel=null; this.goldenTrendAutoWinStreak=0; this.goldenTrendAutoAnchorPrice=null;
    this.superTrendAutoLevel=null;  this.superTrendAutoWinStreak=0;  this.superTrendAutoAnchorPrice=null;
  }
  emit(msg, level='info'){
...
class BacktestRunner {
  ...
  async run() {
    try {
      ...
      const needsTrendBuffers  = cfg.logic === 'super' || cfg.golden_trend_enabled || cfg.golden_consolidation_filter_enabled || cfg.filter === 'trend';
      ...
      // actually needs it (see startWatcher) — Super Logic always feeds it.
      const feedsM1Trend = cfg.logic === 'super' || cfg.filter === 'trend' || cfg.golden_trend_enabled || cfg.golden_consolidation_filter_enabled;
      // M5/M15 buffers: Super Logic always feeds M5 (M15 conditionally); Golden
      // Logic only feeds either when golden_trend_enabled is on.
      const feedsM5M15 = cfg.logic === 'super' || cfg.golden_trend_enabled;
      ...
      for (let i = 0; i < this.allTicks.length; i++) {
        ...
        if (needsCandleQueue) {
          ...
          while (m1Idx < m1Queue.length && m1Queue[m1Idx].epoch + 60 <= tick.epoch) {
            ...
            if (feedsM1Trend) { this.slot.trendBuf.push(candle); if (this.slot.trendBuf.length > 100) this.slot.trendBuf.shift(); }
            if (mainDriveIsCandles) dispatch(this.slot, candle);
            m1Idx++;
          }
          if (feedsM5M15) {
            while (m5Idx < m5Queue.length && m5Queue[m5Idx].epoch + 300 <= tick.epoch) {
              this.slot.superM5Buf.push(m5Queue[m5Idx]); if (this.slot.superM5Buf.length > 100) this.slot.superM5Buf.shift();
              m5Idx++;
            }
            if (wantsM15) {
...
function updateAutoTrendLevel(slot, logic, won){
  ...
  const anchorKey = logic === 'golden' ? 'goldenTrendAutoAnchorPrice': 'superTrendAutoAnchorPrice';
  ...
  if (won) {
    ...
    if (slot[streakKey] >= winStreakNeeded) {
      ...
      slot[streakKey] = 0;
      if (slot[levelKey] !== before) {
        slot[anchorKey] = null; // fresh reference — next tick sets it (see checkAutoTrendPriceDrift)
        slot.emit(`${label} auto-trend: ${winStreakNeeded>1?winStreakNeeded+'-win streak':'win'} → easing to ${slot[levelKey]}%`);
      }
    }
  } else {
    slot[streakKey] = 0;
    ...
    if (slot[levelKey] !== before) {
      slot[anchorKey] = null;
      slot.emit(`${label} auto-trend: loss → tightening to ${slot[levelKey]}%`, 'err');
    }
  }
  slot.pushState();
}
...
/**
 * Second, independent way to ease strictness back down — added per user
 * request. The win-streak mechanism above only fires when a trade actually
 * happens, but at high strictness (e.g. 80%) trades can become rare, so the
 * bot could sit stuck at max strictness through a long quiet stretch with
 * nothing ever firing to prove it should ease. This instead watches raw
 * price movement: if price drifts a configurable number of points in EITHER
 * direction from wherever the level last changed, ease down one step with
 * no trade required at all. Called on every tick/candle (see dispatch()),
 * so it works continuously, not just around trade attempts.
 */
function checkAutoTrendPriceDrift(slot, logic, price){
  const cfg = slot.cfg;
  if ((cfg[`${logic}_trend_strictness_mode`] || 'manual') !== 'automatic') return;
  if (!cfg[`${logic}_trend_auto_price_ease_enabled`]) return;
  if (price == null || isNaN(price)) return;

  const min = Number(cfg[`${logic}_trend_auto_min_pct`]) || 60;
  const points = Number(cfg[`${logic}_trend_auto_price_ease_points`]) || 10;
  const levelKey  = logic === 'golden' ? 'goldenTrendAutoLevel'      : 'superTrendAutoLevel';
  const streakKey = logic === 'golden' ? 'goldenTrendAutoWinStreak'  : 'superTrendAutoWinStreak';
  const anchorKey = logic === 'golden' ? 'goldenTrendAutoAnchorPrice': 'superTrendAutoAnchorPrice';
  const label = logic === 'golden' ? 'Golden' : 'Super';
  ...
  if (slot[levelKey] == null) slot[levelKey] = min;
  if (slot[anchorKey] == null) { slot[anchorKey] = price; return; } // first observation at this level — just set the reference

  const drift = Math.abs(price - slot[anchorKey]);
  if (drift < points) return;

  const before = slot[levelKey];
  slot[levelKey] = Math.max(min, slot[levelKey] - TREND_AUTO_STEP_PCT);
  slot[anchorKey] = price; // reset — next threshold measured fresh from here (stair-steps down on sustained movement)
  slot[streakKey] = 0;
  if (slot[levelKey] !== before) {
    slot.emit(`${label} auto-trend: price moved ${drift.toFixed(2)} pts with no trade → easing to ${slot[levelKey]}%`);
    slot.pushState();
  }
}
...
function dispatch(slot, val){
  ...
  // Runs on every tick/candle regardless of trade state — see
  // checkAutoTrendPriceDrift()'s comment for why this needs to be
  // continuous rather than only checked around trade attempts.
  const genericPrice = (typeof val === 'object' && val !== null) ? parseFloat(val.close) : Number(val);
  checkAutoTrendPriceDrift(slot, slot.cfg.logic === 'super' ? 'super' : 'golden', genericPrice);
...
function startWatcher(slot){
  ...
  // confirmation, and the consolidation filter — one shared subscription for all three.
  if (cfg.filter === 'trend' || cfg.golden_trend_enabled || cfg.golden_consolidation_filter_enabled) {
    slot.candleSubs.push(candleManager.subscribe(cfg.symbol, 60, candle => { if (!slot.running) return; slot.trendBuf.push(candle); if (slot.trendBuf.length>100) slot.trendBuf.shift(); }));
  }
  // M5/M15 are only needed for the new trend confirmation feature — mirrors Super Logic's setup
  if (cfg.golden_trend_enabled) {
    slot.candleSubs.push(candleManager.subscribe(cfg.symbol, 300, candle => { if (!slot.running) return; slot.superM5Buf.push(candle); if (slot.superM5Buf.length>100) slot.superM5Buf.shift(); }));
    if (cfg.golden_trend_mode !== 'm1_5' && cfg.golden_trend_mode !== 'm1_and_5') {
...
async function startSlot(slot){
  ...
  slot.goldenTrendAutoLevel=Number(slot.cfg.golden_trend_auto_min_pct)||60; slot.goldenTrendAutoWinStreak=0; slot.goldenTrendAutoAnchorPrice=null;
  slot.superTrendAutoLevel=Number(slot.cfg.super_trend_auto_min_pct)||60;   slot.superTrendAutoWinStreak=0;  slot.superTrendAutoAnchorPrice=null;
  superReset(slot);
