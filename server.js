...
app.get('/callback', async (req, res) => {
  ...
  try {
    ...
    let accountCurrency = accountsData.data?.[0]?.currency;
    let accountBalance = accountsData.data?.[0]?.balance;
    ...
    if (!accountId) {
      ...
      accountCurrency = createData.data?.[0]?.currency || accountCurrency;
      accountBalance = createData.data?.[0]?.balance ?? accountBalance;
    ...
    if (accountCurrency) acct.currency = accountCurrency; // set immediately — don't wait on the balance push round-trip
    if (accountBalance != null) acct.balance = accountBalance; // same fix, applied to balance — see connectLive()'s periodic refresh too
