...
class Account {
  ...
  openClosedCandleStream(symbol, cb, granularity = 60){
    ...
    ws.on('message', raw => {
      let m; try { m = JSON.parse(raw); } catch { return; }
      // Historical closed candles — feed directly, seed lastOpenTime from last one
      if (m.msg_type === 'candles' && m.candles){
        ...
        if (m.candles.length > 0){
          ...
          lastOpenTime = String(last.epoch); // historical candles use epoch as open_time
          lastOhlc = last;
        }
        return;
      }
      // Live ohlc — fires on every tick for the forming candle
      // open_time is the candle start epoch — only changes when a new candle opens
      if (m.msg_type === 'ohlc' && m.ohlc){
        const ohlc = m.ohlc;
        const openTime = String(ohlc.open_time);
        if (lastOpenTime !== null && openTime !== lastOpenTime && lastOhlc){
          // open_time changed — lastOhlc is now fully closed, fire it
          cb(lastOhlc, true);
        }
...
function startWatcher(slot){
  ...
  if (cfg.logic === 'super'){
    // Super Logic: M1 CLOSED candles only for core logic
    slot.dataWs = acct.openClosedCandleStream(cfg.symbol, (candle, isLive) => {
      if (!slot.running) return;
      // Historical candles — seed M1 trend buffer only, never dispatch as signals
      if (!isLive){ slot.trendBuf.push(candle); if (slot.trendBuf.length > 100) slot.trendBuf.shift(); return; }
      // Closed live candle — update M1 trend buf then dispatch to Super Logic
