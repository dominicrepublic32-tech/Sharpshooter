...
class Account {
  ...
  // Single WebSocket connection for Super Logic — subscribes to M1, M5, M15 on one connection
  openSuperStream(symbol, dbgEmit, onM1Closed, onM5Candle, onM15Candle, trendMode){
    const ws = new WebSocket(WS_DEMO);
    const reqMap = { 1: 60, 2: 300, 3: 900 };
    const state = {};
    [60, 300, 900].forEach(g => { state[g] = { lastOpenTime: null, lastOhlc: null }; });
    let ohlcCount = 0;

    ws.on('open', () => {
      dbgEmit('[DBG] WS connected — sending subscriptions');
      ws.send(JSON.stringify({ ticks_history: symbol, adjust_start_time: 1, count: 50, end: 'latest', granularity: 60,  style: 'candles', subscribe: 1, req_id: 1 }));
      ws.send(JSON.stringify({ ticks_history: symbol, adjust_start_time: 1, count: 50, end: 'latest', granularity: 300, style: 'candles', subscribe: 1, req_id: 2 }));
      if (trendMode !== 'm1_5'){
        ws.send(JSON.stringify({ ticks_history: symbol, adjust_start_time: 1, count: 50, end: 'latest', granularity: 900, style: 'candles', subscribe: 1, req_id: 3 }));
      }
    });
    ...
    ws.on('message', raw => {
      ...
      if (m.msg_type === 'candles' && m.candles && m.candles.length > 0){
        const reqId = m.req_id || (m.echo_req && m.echo_req.req_id);
        const g     = reqMap[reqId] || 60;
        state[g].lastOpenTime = String(m.candles[m.candles.length - 1].epoch);
        dbgEmit(`[DBG] candles snapshot: req_id=${reqId} → g=${g} count=${m.candles.length} lastEpoch=${m.candles[m.candles.length-1].epoch}`);
        if (g === 300) m.candles.forEach(c => onM5Candle(c));
        if (g === 900) m.candles.forEach(c => onM15Candle(c));
        return;
      }
      ...
      if (m.msg_type === 'ohlc' && m.ohlc){
        ...
        const openTime = String(ohlc.open_time);
        const s        = state[g];
        if (!s) return;
        // Log every 30th ohlc tick so we can see stream is alive
        ohlcCount++;
        if (ohlcCount % 30 === 1) dbgEmit(`[DBG] ohlc tick #${ohlcCount}: g=${g} open_time=${openTime} lastOpenTime=${s.lastOpenTime} close=${ohlc.close}`);

        if (s.lastOpenTime !== null && openTime !== s.lastOpenTime && s.lastOhlc){
          dbgEmit(`[DBG] CANDLE CLOSED g=${g} open_time changed ${s.lastOpenTime}→${openTime}`);
          if (g === 60)  onM1Closed(s.lastOhlc);
          if (g === 300) onM5Candle(s.lastOhlc);
          if (g === 900) onM15Candle(s.lastOhlc);
        }
        s.lastOpenTime = openTime;
        s.lastOhlc     = ohlc;
      }

      // Log unknown message types
      if (m.msg_type !== 'ohlc' && m.msg_type !== 'candles'){
        dbgEmit(`[DBG] other msg: ${m.msg_type} err=${m.error ? m.error.message : 'none'}`);
      }
    });
...
function startWatcher(slot){
  ...
  if (cfg.logic === 'super'){
    const dbgEmit = (msg) => slot.emit(msg);
    slot.dataWs = acct.openSuperStream(
      cfg.symbol,
      dbgEmit,
      // M1 closed candle — core logic
      (candle) => {
        if (!slot.running) return;
        slot.trendBuf.push(candle); if (slot.trendBuf.length > 100) slot.trendBuf.shift();
        slot.emit(`[DBG] M1 closed: open=${candle.open} close=${candle.close} open_time=${candle.open_time}`);
        dispatch(slot, candle);
      },
      // M5 candle — trend buffer only
      (candle) => {
        if (!slot.running) return;
        slot.emit(`[DBG] M5 candle received open_time=${candle.open_time}`);
        slot.superM5Buf.push(candle); if (slot.superM5Buf.length > 100) slot.superM5Buf.shift();
      },
      // M15 candle — trend buffer only
      (candle) => {
        if (!slot.running) return;
        slot.emit(`[DBG] M15 candle received open_time=${candle.open_time}`);
        slot.superM15Buf.push(candle); if (slot.superM15Buf.length > 100) slot.superM15Buf.shift();
      },
      cfg.super_trend_mode
    );
    slot.superM5Ws  = null; // single connection handles all
    slot.superM15Ws = null;
