...
class Account {
  ...
  openSuperStream(symbol, onM1Closed, onM5Candle, onM15Candle, trendMode){
    ...
    ws.on('open', () => {
      ...
      // Subscribe M5 (trend)
      ws.send(JSON.stringify({ ticks_history: symbol, adjust_start_time: 1, count: 50, end: 'latest', granularity: 300, style: 'candles', subscribe: 1 }));
      // Subscribe M15 (trend — only in m1_5_15 mode)
      if (trendMode !== 'm1_5'){
        ws.send(JSON.stringify({ ticks_history: symbol, adjust_start_time: 1, count: 50, end: 'latest', granularity: 900, style: 'candles', subscribe: 1 }));
      }
    });
    ...
    ws.on('message', raw => {
      ...
      // Historical snapshots — route by granularity
      if (m.msg_type === 'candles' && m.candles && m.candles.length > 0){
        const g = m.candles[0].granularity ? Number(m.candles[0].granularity) : 60;
        // Seed lastOpenTime from last historical candle
        if (state[g]) state[g].lastOpenTime = String(m.candles[m.candles.length - 1].epoch);
        // Feed historical candles to trend buffers (M5/M15 only — M1 history not used as signals)
        if (g === 300) m.candles.forEach(c => onM5Candle(c));
        if (g === 900) m.candles.forEach(c => onM15Candle(c));
        return;
      }

      // Live ohlc — detect closed candle via open_time change
      if (m.msg_type === 'ohlc' && m.ohlc){
        ...
        if (s.lastOpenTime !== null && openTime !== s.lastOpenTime && s.lastOhlc){
          // Candle closed — fire with final OHLC values
