...
class Account {
  ...
  openClosedCandleStream(symbol, cb, granularity = 60){
    ...
    ws.on('message', raw => {
      let m; try { m = JSON.parse(raw); } catch { return; }
      // Historical closed candles — feed directly
      if (m.msg_type === 'candles' && m.candles){
        m.candles.forEach(c => cb(c, false));
        // Seed lastOhlc with the most recent historical candle so we have a baseline epoch
        if (m.candles.length > 0) lastOhlc = m.candles[m.candles.length - 1];
        return;
      }
      // Live ohlc tick — fires on every tick for the currently forming candle
      if (m.msg_type === 'ohlc' && m.ohlc){
        const ohlc = m.ohlc;
        if (lastOhlc && String(ohlc.epoch) !== String(lastOhlc.epoch)){
          // Epoch changed — lastOhlc is now the closed candle, fire it
          cb(lastOhlc, true);
        }
        lastOhlc = ohlc; // always keep latest forming candle
      }
    });
    ws.on('error', () => {}); return ws;
  }
