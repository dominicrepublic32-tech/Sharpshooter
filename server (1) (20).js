...
function startWatcher(slot){
  ...
  if (cfg.logic === 'super'){
    // Super Logic: M1 closed candles for core logic
    slot.dataWs = acct.openCandleStream(cfg.symbol, (candle, isLive) => {
      if (!slot.running) return;
      // Feed historical candles into trendBuf for M1 trend — never dispatch them as signals
      if (!isLive){ slot.trendBuf.push(candle); if (slot.trendBuf.length > 100) slot.trendBuf.shift(); return; }
      // Live (closed) candle — update M1 trend buf then dispatch to Super Logic
      slot.trendBuf.push(candle); if (slot.trendBuf.length > 100) slot.trendBuf.shift();
      dispatch(slot, candle);
    }, 60);

    // M5 stream — trend detection only
    slot.superM5Ws = acct.openCandleStream(cfg.symbol, (candle) => {
      if (!slot.running) return;
      slot.superM5Buf.push(candle); if (slot.superM5Buf.length > 100) slot.superM5Buf.shift();
    }, 300);

    // M15 stream — trend detection only (only needed in m1_5_15 mode)
    if (cfg.super_trend_mode !== 'm1_5'){
      slot.superM15Ws = acct.openCandleStream(cfg.symbol, (candle) => {
        if (!slot.running) return;
