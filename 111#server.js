...
const DATAENGINE_MARKETS = ['1HZ100V', '1HZ75V', '1HZ50V', '1HZ25V', '1HZ10V']; // Volatility 100/75/50/25/10 (1s)
...
class Account {
  ...
  // Fixed (added) — watchContract()'s subscription to a specific contract's
  // updates lives on whatever WebSocket was open when the trade fired. If
  // that socket ever reconnects mid-trade (which the connection watchdog
  // makes more likely by design), the old subscription silently dies with
  // it — nothing re-establishes it on the new socket, so the watcher just
  // sits waiting forever for an update that will never come, until it times
  // out — REGARDLESS of what actually happened to the trade on Deriv's side.
  // This re-sends the subscribe request for every still-pending watcher the
  // moment a new connection is ready, so long-duration trades (e.g. 3h
  // ONETOUCH) survive a reconnect during their hold instead of going blind.
  _resubscribeWatchers(){
    if (this.watchers.size === 0) return;
    serverLog(`Account reconnected — re-subscribing to ${this.watchers.size} pending contract watcher(s)`);
    this.watchers.forEach((_cb, contractId) => {
      this.send({ proposal_open_contract: 1, contract_id: contractId, subscribe: 1 }).catch(() => {});
    });
  }

  watchContract(id, onUpdate, timeoutMs){
    return new Promise((resolve, reject) => {
      let maxSpot = null, minSpot = null, entrySpot = null, barrier = null;
      const t = setTimeout(async () => {
        this.watchers.delete(id);
        // Second, independent safety net (added) — before giving up and
        // reporting a failure (which the caller treats as an unrecorded
        // loss, not a real settlement), ask Deriv directly, ONE time,
        // non-subscribed: "what is this contract's actual status right
        // now?" This catches ANY reason the subscription went silent (a
        // reconnect the resubscribe fix above didn't cover, a dropped
        // message, anything) — not just the specific reconnect case. If
        // Deriv confirms it's actually settled, resolve normally with the
        // real result instead of erroring out.
        try {
          const r = await this.send({ proposal_open_contract: 1, contract_id: id, subscribe: 0 });
          const c = r?.proposal_open_contract;
          if (c && (c.is_sold || c.is_expired)) {
            c._maxSpot = maxSpot; c._minSpot = minSpot; c._entrySpot = entrySpot ?? (c.entry_spot != null ? parseFloat(c.entry_spot) : null); c._barrier = barrier ?? (c.barrier != null ? parseFloat(c.barrier) : null);
            resolve(c);
            return;
          }
        } catch {}
      ...
      // Fixed (added) — this used to only .catch() network-level failures
      // and never looked at the RESOLVED response itself. If Deriv responded
      // normally but declined the subscription (e.g. {error:{...}} in the
      // payload — no network problem, no disconnect, nothing to catch), the
      // watcher above sat registered forever waiting for updates tied to a
      // subscription that was never actually established, until the full
      // timeout. Now the response is checked, and a rejected/failed
      // subscribe is retried a few times (a brief transient issue on
      // Deriv's side shouldn't cost the whole trade) before giving up.
      const trySubscribe = (attempt) => {
        this.send({ proposal_open_contract: 1, contract_id: id, subscribe: 1 })
          .then(r => {
            if (r?.error) {
              if (attempt < 3) { setTimeout(() => trySubscribe(attempt + 1), 2000); return; }
              serverLog('watchContract: subscribe rejected after retries', { contractId: id, err: r.error.message });
              // Don't reject outright — the timeout's final direct-poll safety
              // net above can still recover this if the contract settles
              // before the timeout, so just let it ride rather than aborting.
            }
          })
          .catch(e => {
            if (attempt < 3) { setTimeout(() => trySubscribe(attempt + 1), 2000); return; }
            clearTimeout(t); this.watchers.delete(id); reject(e);
          });
      };
      trySubscribe(1);
    });
...
class CandleManager {
  ...
  subscribe(symbol, granularity, cb){
    ...
    if (!g) { g = { current:null, subs:new Set(), backfilled:false, lastEmittedEpoch:null }; entry.grans.set(granularity, g); }
    g.subs.add(cb);
  ...
  _onTick(symbol, tick){
    ...
    entry.grans.forEach((g, granularity) => {
      ...
      if (windowStart > g.current.epoch) {
        ...
        g.lastEmittedEpoch = completed.epoch;
  ...
  async _backfill(symbol, granularity){
    ...
    try {
      ...
      // CRITICAL FIX (added): this used to unconditionally replay every one of
      // the last ~49 candles through the SAME live callbacks used for real-time
      // trade firing — on every reconnect, not just the first-ever subscribe.
      // Combined with the more aggressive reconnect/watchdog behavior added
      // elsewhere, a run of reconnects could replay the same recent candles
      // multiple times, causing the exact same qualifying setup to fire more
      // than once for what was really only ONE genuine candle close — this is
      // very likely the cause of trades clustering at/near the same timestamp.
      // Fix: only emit candles this granularity hasn't already emitted before
      // (epoch > lastEmittedEpoch) — i.e. backfill now only fills a genuine
      // gap from time spent disconnected, never replays what's already been
      // seen. On the very first-ever subscribe (lastEmittedEpoch is still
      // null), everything in the history is legitimately new, so it all goes
      // through as before.
      const toEmit = g.lastEmittedEpoch == null
        ? candles.slice(0, -1)
        : candles.slice(0, -1).filter(c => c.epoch > g.lastEmittedEpoch);
      toEmit.forEach(c => { g.subs.forEach(cb => { try { cb(c); } catch (e) {} }); g.lastEmittedEpoch = c.epoch; });
...
// Auto-start on boot REMOVED (was liveTickCollector.start(DATAENGINE_MARKETS)) —
// continuous background collection is now OFF by default for every market.
// Nothing runs automatically; use the Resume button (or POST
// /api/data/live/:symbol/resume) per market to manually turn collection on.
// One-off historical downloads (POST /api/data/download) were already
// fully manual before this change and are unaffected.
serverLog('DataEngine ready (live collection off by default — resume manually per market)', { dbPath: DATAENGINE_DB_PATH, markets: DATAENGINE_MARKETS });
...
async function estimatePayout(params){
  try {
    // Same fix as queryProposalPayout below — Deriv's proposal schema wants
    // 'underlying_symbol' instead of 'symbol' on a Live-authenticated
    // connection. This was silently masked before by the fallback estimate
    // below, so Demo Mode's displayed payout was quietly using the rough
    // guess instead of a real quote whenever acct.isLive was true.
    const symbolField = acct.isLive ? { underlying_symbol: params.symbol } : { symbol: params.symbol };
    const r = await acct.send({
      ...
      ...symbolField, ...(params.barrier != null ? { barrier:params.barrier } : {}),
    });
...
// ─── Rotative Barrier (added) ───────────────────────────────────────────────
// A second barrier mode alongside the existing fixed barrier1/barrier2 (that
// path is completely untouched — see resolveTradeBarrier below). Instead of
// a barrier you type in, this computes the barrier fresh before every trade
// by querying Deriv's REAL proposal pricing and searching for the exact
// distance that makes payout === stake + your target profit. "Exact" is
// taken literally: if the search can't land on the target to the cent
// within the iteration budget, it does NOT fire with an approximate
// barrier — it skips that trade and says so.
//
// Unlike estimatePayout() above (which has a rough guess fallback for Demo
// Mode's display purposes), this NEVER guesses — a pricing failure means
// "give up, don't fire," full stop.
//
// Only works where a live Deriv connection actually exists to price
// against: real Live accounts, real Deriv Demo accounts, and the bot's own
// local Demo Mode (all three keep `acct` connected for this purpose). It
// CANNOT work in Backtest — there's no live pricing engine to query for a
// past date — so Backtest explicitly refuses this mode rather than either
// silently querying today's real price (wrong) or silently producing zero
// trades with no explanation.
async function queryProposalPayout(symbol, contractType, stake, duration, durationUnit, barrierStr){
  // Fixed (added) — Deriv's proposal schema uses a DIFFERENT field name for
  // the symbol depending on connection type: 'underlying_symbol' for a Live-
  // authenticated connection, 'symbol' otherwise. This exactly mirrors what
  // the proven, already-working buy() method does above — using the wrong
  // one gets the whole request rejected with "Properties not allowed: symbol."
  const symbolField = acct.isLive ? { underlying_symbol: symbol } : { symbol };
  const r = await acct.send({
    proposal: 1, amount: Number(stake), basis: 'stake', contract_type: contractType,
    currency: acct.currency || 'USD', duration: Number(duration), duration_unit: durationUnit,
    ...symbolField, barrier: barrierStr,
  });
  if (r.error) throw new Error(r.error.message);
  const payout = parseFloat(r.proposal?.payout);
  if (isNaN(payout) || payout <= 0) throw new Error('No payout in proposal response');
  return payout;
}

// Binary search over barrier OFFSET (the magnitude after the +/- sign).
// Payout increases monotonically as the barrier moves further from the
// current spot (further = less likely to touch = bigger payout needed for
// the same stake to be fair) — that monotonicity is what makes this search
// valid. Returns { barrierStr, payout } on an exact match, or null if no
// exact match was found (pricing failed, couldn't bracket the target, or
// ran out of iterations/precision).
// tolerancePct is the acceptable band width — e.g. 5 means anything between
// 95% and 105% of targetPayout is accepted immediately, no need to keep
// narrowing toward an exact cent match. Always returns the CLOSEST match it
// found once iterations run out (never returns null just because it didn't
// land inside the band in time) — the only null returns left are genuine
// failures: can't price at all, or can't bracket the target in the first
// place. bestRes tracks the closest-to-target result across every priced
// point so there's always something usable to fall back to.
async function findRotativeBarrier(symbol, sign, stake, duration, durationUnit, targetPayout, maxIterations, contractType, tolerancePct, anchorOffset){
  const band = targetPayout * (Math.max(0, tolerancePct) / 100);
  const withinBand = (payout) => Math.abs(payout - targetPayout) <= band;

  const priceAt = async (offset) => {
    const barrierStr = `${sign}${offset.toFixed(2)}`;
    const payout = await queryProposalPayout(symbol, contractType, stake, duration, durationUnit, barrierStr);
    return { barrierStr, payout };
  };

  let bestRes = null;
  const track = (res) => { if (!bestRes || Math.abs(res.payout - targetPayout) < Math.abs(bestRes.payout - targetPayout)) bestRes = res; };

  // Starts from the user's OWN fixed Barrier (+)/(−) value (added) — that
  // value is already proven valid for this symbol (it's what Normal mode
  // trades with directly), so it's a far better first guess than any
  // arbitrary number, and sidesteps Deriv rejecting an unrealistically tiny
  // barrier outright. A small fallback ladder around it covers the rare case
  // where this exact duration/contract combo doesn't accept that precise
  // value even though the field itself is generally valid.
  const base = Math.max(0.01, Math.abs(Number(anchorOffset)) || 1);
  const startCandidates = [base, base * 1.5, base * 0.5, base * 2, base * 0.25, base * 4];
  let anchor, anchorRes, lastErr, iterations = 0;
  for (const candidate of startCandidates) {
    try { anchorRes = await priceAt(candidate); anchor = candidate; iterations++; break; }
    catch (e) { lastErr = e.message; iterations++; continue; }
  }
  if (!anchorRes) { findRotativeBarrier.lastError = lastErr || 'unknown error'; return null; } // every starting point failed — genuine failure
  track(anchorRes);
  if (withinBand(anchorRes.payout)) return anchorRes;

  // Payout increases with distance from spot, so which direction to search
  // depends on whether the anchor undershot or overshot the target —
  // undershoot means go bigger (expand upward from anchor as the low bound);
  // overshoot means go smaller (shrink downward from anchor as the high bound).
  let lo, hi, loRes, hiRes;
  if (anchorRes.payout < targetPayout) {
    lo = anchor; loRes = anchorRes;
    hi = lo * 2;
    try { hiRes = await priceAt(hi); iterations++; }
    catch { return bestRes; }
    track(hiRes);
    if (withinBand(hiRes.payout)) return hiRes;
    let expandAttempts = 0;
    while (hiRes.payout < targetPayout && expandAttempts < 15 && iterations < maxIterations){
      hi *= 2;
      try { hiRes = await priceAt(hi); } catch { return bestRes; }
      track(hiRes);
      if (withinBand(hiRes.payout)) return hiRes;
      expandAttempts++; iterations++;
    }
    if (hiRes.payout < targetPayout) return bestRes; // couldn't bracket the target within the expansion/iteration budget — best effort
  } else {
    hi = anchor; hiRes = anchorRes;
    lo = hi / 2;
    try { loRes = await priceAt(lo); iterations++; }
    catch { return bestRes; }
    track(loRes);
    if (withinBand(loRes.payout)) return loRes;
    let shrinkAttempts = 0;
    while (loRes.payout > targetPayout && shrinkAttempts < 15 && iterations < maxIterations && lo > 0.005){
      lo /= 2;
      try { loRes = await priceAt(lo); } catch { return bestRes; }
      track(loRes);
      if (withinBand(loRes.payout)) return loRes;
      shrinkAttempts++; iterations++;
    }
    if (loRes.payout > targetPayout) return bestRes; // couldn't shrink small enough — best effort
  }

  while (iterations < maxIterations && (hi - lo) > 0.0001){
    const mid = (lo + hi) / 2;
    let midRes;
    try { midRes = await priceAt(mid); } catch { hi = mid; iterations++; continue; } // treat a pricing error as "too far", narrow from the top
    iterations++;
    track(midRes);
    if (withinBand(midRes.payout)) return midRes;
    if (midRes.payout < targetPayout) lo = mid; else hi = mid;
  }
  return bestRes; // iteration budget exhausted — fire with the closest match found rather than skipping
}

// The single entry point both firing paths (Golden's fireReversal, Super's
// superFireTrade) call. sign is '+' or '-'; contractType is whatever this
// trade's actual contract type is (only barrier-using types are affected —
// see NEEDS_BARRIER). Returns the barrier STRING to use, or null only on a
// genuine failure (no live connection to price against, or Backtest, where
// rotative mode isn't available at all) — NOT for "not close enough", since
// this now always fires with the closest match it managed to find.
async function resolveTradeBarrier(slot, sign, contractType){
  const c = slot.cfg;
  const normalBarrier = sign === '+' ? c.barrier1 : c.barrier2;
  if (!NEEDS_BARRIER.has(contractType)) return normalBarrier; // this contract type doesn't use a barrier at all
  if ((c.barrier_mode || 'normal') !== 'rotative') return normalBarrier;

  if (slot.backtest) {
    slot.emit('Rotative Barrier is not available in Backtest (no live Deriv pricing to query for a past date) — skipping this trade. Switch to Normal barrier mode to backtest.', 'err');
    return null;
  }

  const targetProfit = Number(c.rotative_target_profit) || 0;
  const targetPayout = Number(c.stake) + targetProfit;
  const maxIter = Math.max(5, Number(c.rotative_max_iterations) || 40);
  const tolerancePct = Math.max(0, Number(c.rotative_tolerance_pct) ?? 5);
  const anchorOffset = parseFloat(normalBarrier) || 1; // start from the user's own fixed Barrier (+)/(−) value
  findRotativeBarrier.lastError = null;
  const result = await findRotativeBarrier(c.symbol, sign, c.stake, c.duration_value, c.duration_unit, targetPayout, maxIter, contractType, tolerancePct, anchorOffset)
    .catch((e) => { findRotativeBarrier.lastError = e.message; return null; });
  if (!result) {
    const reason = findRotativeBarrier.lastError || 'unknown error';
    slot.emit(`Rotative Barrier: couldn't get a live price quote — ${reason} — skipping this trade`, 'err');
    return null;
  }
  const pctOfTarget = (result.payout / targetPayout) * 100;
  const inBand = pctOfTarget >= (100 - tolerancePct) && pctOfTarget <= (100 + tolerancePct);
  slot.emit(`Rotative Barrier: ${result.barrierStr} → payout $${result.payout.toFixed(2)} (target $${targetPayout.toFixed(2)}, ${pctOfTarget.toFixed(1)}% of target${inBand ? '' : ' — outside ±' + tolerancePct + '% band, firing with closest found'})`);
  return result.barrierStr;
}
...
async function runContract(slot, params, label, opts = {}){
  ...
  if (backtest && ssCtrl) {
    ...
    if (ssCtrl) ssCtrl.contractId = cid;
    // Watch timeout (fixed — was hardcoded to 5 minutes, which silently broke
    // stats/history for any trade longer than that, e.g. the 3h ONETOUCH
    // trades from Task 1). Now sized to the ACTUAL contract duration plus a
    // 10-minute safety buffer for settlement/network lag. Tick-based
    // durations ('t') have no fixed wall-clock length, so those keep the
    // original 5-minute default — unchanged from before. (Local mapping here
    // — NOT reusing resolveContractFromHistory's same-named constant, which
    // is scoped inside that function only and isn't reachable from here.)
    const DURATION_UNIT_SECONDS_LOCAL = { s: 1, m: 60, h: 3600, d: 86400 };
    const durSecondsForWatch = DURATION_UNIT_SECONDS_LOCAL[params.duration_unit] ? params.duration * DURATION_UNIT_SECONDS_LOCAL[params.duration_unit] : null;
    const watchTimeoutMs = durSecondsForWatch ? Math.max(5*60*1000, durSecondsForWatch*1000 + 10*60*1000) : 5*60*1000;
    settled = await acct.watchContract(cid, ssCtrl ? ({ entrySpot }) => {
      if (ssCtrl.entrySpot == null && entrySpot != null) ssCtrl.entrySpot = entrySpot;
    } : undefined, watchTimeoutMs);
  }
...
// ─── slot defaults ────────────────────────────────────────────────────────────
// Dedicated per-slot markets (added) — each of the 10 slots now defaults to
// a specific market instead of every slot starting on 1HZ100V. With only 5
// markets and 10 slots, it cycles: slots 0&5 -> V100, 1&6 -> V75, 2&7 -> V50,
// 3&8 -> V25, 4&9 -> V10 (2 slots per market). This is just the STARTING
// default for a fresh slot — the Market dropdown is untouched and still
// lets you change any slot to any market afterward.
const SLOT_DEFAULT_MARKETS = ['1HZ100V', '1HZ75V', '1HZ50V', '1HZ25V', '1HZ10V'];

function defaultCfg(idx){
  const symbol = SLOT_DEFAULT_MARKETS[(Number(idx) || 0) % SLOT_DEFAULT_MARKETS.length] || '1HZ100V';
  return {
    symbol, logic:'golden', observe_mode:'ticks', observe_count:5, reaction_range:0.3,
    confirm_count:1, stake:1, duration_value:5, duration_unit:'t',
    barrier1:'+0.25', barrier2:'-0.25', contract_type:'ONETOUCH',
    // Rotative Barrier (added) — a second barrier mode alongside the fixed
    // barrier1/barrier2 above (untouched, still used as-is when barrier_mode
    // is 'normal'). 'rotative' computes the barrier fresh before EVERY trade
    // by querying Deriv's real-time proposal pricing, searching for the
    // exact barrier distance that makes payout === stake + rotative_target_profit.
    // Requires a live Deriv connection to price against — NOT available
    // during backtest (no live pricing engine to query for past dates); see
    // resolveTradeBarrier()/findRotativeBarrier().
    barrier_mode:'normal',
    rotative_target_profit:0,
    rotative_max_iterations:40,
    rotative_tolerance_pct:5,
    // Daily Candle Color Filter (added) — a standalone feature, separate
    // from the other multi-timeframe filters (doesn't share their timeframe
    // picker; it's always Daily/D1). Looks back over the last N closed D1
    // candles, counts how many are green vs red (own open vs own close),
    // and requires at least `required` of them to match before allowing a
    // + barrier (needs green majority) or − barrier (needs red majority).
    // See dailyCandleColorFilterOk().
    daily_candle_filter_enabled:false,
    daily_candle_filter_lookback:3,
    daily_candle_filter_required:2,
    ...
    // Super Logic's "Automatic" strictness mode was REPLACED (added) — it no
    // longer reacts to win/loss streaks at all. Instead: starts at
    // super_trend_auto_max_pct (highest/strictest). Once super_trend_auto_fire_count
    // trades have FIRED (any outcome — firing itself is the trigger, not
    // settlement), it drops to super_trend_auto_min_pct for trades after that.
    // super_trend_auto_loss_reset CONSECUTIVE losses in a row (any win breaks
    // the streak back to 0) resets everything back to the highest %. Golden
    // Logic's Automatic mode is completely untouched — still the original
    // win-streak-easing behavior above. See getEffectiveTrendThreshold() /
    // updateAutoTrendLevel() / registerSuperCascadeFire().
    super_trend_auto_fire_count:3,
    super_trend_auto_loss_reset:1,
    super_trend_auto_direction_gate_enabled:false,
    ...
    // Rotating Efficiency (added) — a second mode for the same filter, fully
    // separate from super_efficiency_min_ratio above (that field is untouched
    // and still used as-is when mode is 'min'). 'min' = old single-threshold
    // behavior (default, so existing setups don't change). 'rotating' = the
    // two-threshold alternating behavior — see superEfficiencyOk().
    super_efficiency_mode:'min',
    super_efficiency_rotate_max_ratio:60,
    super_efficiency_rotate_min_ratio:25,
...
class Slot {
  constructor(idx){
    this.idx=idx; this.id=`slot${idx}`; this.cfg=defaultCfg(idx);
    ...
    this.structM30Buf=[]; this.structH1Buf=[]; this.structH4Buf=[]; this.structD1Buf=[];
    ...
    // Super cascade state (added) — only used when super_trend_strictness_mode
    // ==='automatic'. Starts 'high' (strictest). See updateAutoTrendLevel(),
    // getEffectiveTrendThreshold(), and registerSuperCascadeFire().
    this.superCascadeLevel='high';
    this.superCascadeFireCount=0;
    this.superCascadeLossStreak=0;
    this.superCascadeOpenTrades=[]; // rolling reference (added) — [{id, price, direction, fireTime}], oldest still-open trade is the gate's reference
    // Rotating Efficiency state (added) — only used when
    // super_efficiency_mode==='rotating'. Starts at 'max' ("bot starts with
    // max efficiency"). See superEfficiencyOk() for the full state machine.
    this.superEffRotateLevel='max';
...
class BacktestRunner {
  ...
  async run() {
    try {
      ...
      // Daily Candle Color Filter (added) — self-contained, own toggle.
      const structureWantsD1  = !!cfg.daily_candle_filter_enabled;
      ...
      const needsCandleQueue   = mainDriveIsCandles || needsTrendBuffers || structureWantsD1;
      ...
      const d1Queue  = structureWantsD1  ? aggregateCandles(m1Queue, 86400) : [];
      ...
      let m1Idx = 0, m5Idx = 0, m15Idx = 0, m30Idx = 0, h1Idx = 0, h4Idx = 0, d1Idx = 0;
      ...
      for (let i = 0; i < this.allTicks.length; i++) {
        ...
        if (needsCandleQueue) {
          ...
          if (structureWantsD1) {
            while (d1Idx < d1Queue.length && d1Queue[d1Idx].epoch + 86400 <= tick.epoch) {
              this.slot.structD1Buf.push(d1Queue[d1Idx]); if (this.slot.structD1Buf.length > 60) this.slot.structD1Buf.shift();
              d1Idx++;
            }
          }
        }
...
function onConfirmTick(slot, price){
  ...
  if (slot.confSide==='high' && (slot.confBuf.filter(p=>p<slot.zoneHigh).length>=slot.cfg.confirm_count || slot.cfg.confirm_count===0)) fireReversal(slot,'high').catch(e => { slot.emit('Golden fire error: '+e.message, 'err'); slot.busy = false; });
  if (slot.confSide==='low'  && (slot.confBuf.filter(p=>p>slot.zoneLow ).length>=slot.cfg.confirm_count || slot.cfg.confirm_count===0)) fireReversal(slot,'low').catch(e => { slot.emit('Golden fire error: '+e.message, 'err'); slot.busy = false; });
}

async function fireReversal(slot, side){
  const c = slot.cfg;
  ...
  // Daily Candle Color Filter (added) — 'high' side = + barrier = BUY (needs
  // green D1 majority); 'low' side = − barrier = SELL (needs red D1 majority).
  if (!dailyCandleColorFilterOk(slot, side==='high' ? 'buy' : 'sell')) {
    slot.phase='watching'; slot.zoneHigh=null; slot.zoneLow=null; slot.confSide=null; slot.confBuf=[];
    slot.pushState();
    return;
  }
  ...
  const barrier = await resolveTradeBarrier(slot, side==='high' ? '+' : '-', 'ONETOUCH');
  if (barrier == null) {
    // Rotative Barrier couldn't find an exact match (or Backtest, where it's
    // unavailable) — resolveTradeBarrier already logged why. Don't fire.
    slot.phase='watching'; slot.busy=false; slot.zoneHigh=null; slot.zoneLow=null; slot.confSide=null; slot.confBuf=[];
    slot.pushState();
    return;
  }
...
function getEffectiveTrendThreshold(slot, logic){
  ...
  // Super Logic's Automatic mode was replaced with the fire-count cascade
  // (added) — see getSuperCascadeThreshold(). Golden Logic below is untouched.
  if (logic === 'super') return getSuperCascadeThreshold(slot);
...
function updateAutoTrendLevel(slot, logic, won){
  ...
  // Super Logic's Automatic mode was replaced with the fire-count cascade
  // (added) — see updateSuperCascadeOnSettle(). Golden Logic below is untouched.
  if (logic === 'super') { updateSuperCascadeOnSettle(slot, won); return; }
...
// ─── Super Logic Automatic-mode cascade (added) ────────────────────────────
// Replaces the old win/loss-streak behavior for Super only — Golden's
// Automatic mode above is completely unaffected.
//   • Starts at 'high' (super_trend_auto_max_pct) every time the bot starts.
//   • registerSuperCascadeFire() — called every time a Super trade actually
//     FIRES (see superFireTrade) — counts fires regardless of outcome. Once
//     super_trend_auto_fire_count fires have happened, it drops to 'low'
//     (super_trend_auto_min_pct) for every trade after that.
//   • updateSuperCascadeOnSettle() — called once a trade's win/loss is known.
//     Wins are neutral (only reset the loss streak). super_trend_auto_loss_reset
//     CONSECUTIVE losses in a row (a win breaks the streak back to 0) resets
//     the whole cascade back to 'high'.
//   • superCascadeDirectionOk() — optional (super_trend_auto_direction_gate_enabled).
//     Once loosened, only allows trades on the correct side of the very FIRST
//     fired trade's entry price: below it if that trade was SELL, above it
//     if BUY. Independent of the fire-count/reset mechanics above — can be
//     combined or left off.
function getSuperCascadeThreshold(slot){
  const cfg = slot.cfg;
  const min = Number(cfg.super_trend_auto_min_pct) || 60; // Loose %
  const max = Number(cfg.super_trend_auto_max_pct) || 80; // Highest % (start/reset)
  return slot.superCascadeLevel === 'low' ? min : max;
}

function updateSuperCascadeOnSettle(slot, won){
  if (won) { slot.superCascadeLossStreak = 0; return; } // wins are neutral — only break the loss streak
  const needed = Math.max(1, Number(slot.cfg.super_trend_auto_loss_reset) || 1);
  slot.superCascadeLossStreak = (slot.superCascadeLossStreak || 0) + 1;
  if (slot.superCascadeLossStreak >= needed) {
    slot.superCascadeLevel = 'high';
    slot.superCascadeFireCount = 0;
    slot.superCascadeLossStreak = 0;
    // Note: superCascadeOpenTrades is NOT touched here — that tracker
    // reflects which trades are actually still open on Deriv, independent of
    // this loss-streak-driven level reset (kept as two separate mechanisms
    // per explicit instruction).
    slot.emit(`Super auto-trend: ${needed > 1 ? needed + ' losses in a row' : 'loss'} → reset to highest %`, 'err');
    slot.pushState();
  }
}

function registerSuperCascadeFire(slot, direction, price){
  const cfg = slot.cfg;
  if ((cfg.super_trend_strictness_mode || 'manual') !== 'automatic') return;
  slot.superCascadeFireCount = (slot.superCascadeFireCount || 0) + 1;
  const needed = Math.max(1, Number(cfg.super_trend_auto_fire_count) || 3);
  if (slot.superCascadeLevel === 'high' && slot.superCascadeFireCount >= needed) {
    slot.superCascadeLevel = 'low';
    slot.emit(`Super auto-trend: ${needed} trade${needed>1?'s':''} fired → loosening to ${Number(cfg.super_trend_auto_min_pct)||60}%`);
    slot.pushState();
  }
}

// ─── Rolling directional reference (rebuilt) ───────────────────────────────
// The gate no longer anchors to a single fixed "first trade" forever. Instead
// it always compares against whichever still-OPEN trade is the OLDEST right
// now. Concretely:
//   • pushSuperCascadeOpenTrade() — called at fire time, adds this trade to
//     the open list and returns a token.
//   • popSuperCascadeOpenTrade() — called once that specific trade settles
//     (win or loss), removes it from the open list.
//   • getSuperCascadeReference() — the oldest entry still in the open list,
//     or null if nothing is open right now.
// Net effect: while Trade A is open, it's the reference. A new trade (B) is
// checked against A, then joins the open list too. Once A settles and is
// removed, B — if still open — automatically becomes the reference for
// whatever fires next (no manual "promote" step needed, since the reference
// is always just "whichever open trade is oldest" at check time). If nothing
// at all is open, there's no reference, so the gate imposes no constraint —
// and whatever fires next becomes the new (and only) reference by simply
// being the oldest thing in an otherwise-empty list.
let _superCascadeTradeSeq = 0;
function pushSuperCascadeOpenTrade(slot, direction, price){
  const cfg = slot.cfg;
  if ((cfg.super_trend_strictness_mode || 'manual') !== 'automatic') return null;
  if (price == null || isNaN(price)) return null;
  const token = { id: ++_superCascadeTradeSeq, direction, price, fireTime: Date.now() };
  slot.superCascadeOpenTrades.push(token);
  return token;
}
function popSuperCascadeOpenTrade(slot, token){
  if (!token || !slot.superCascadeOpenTrades) return;
  const i = slot.superCascadeOpenTrades.findIndex(t => t.id === token.id);
  if (i !== -1) slot.superCascadeOpenTrades.splice(i, 1);
}
function getSuperCascadeReference(slot){
  const list = slot.superCascadeOpenTrades;
  if (!list || list.length === 0) return null;
  let oldest = list[0];
  for (let i = 1; i < list.length; i++) if (list[i].fireTime < oldest.fireTime) oldest = list[i];
  return oldest;
}

function superCascadeDirectionOk(slot, currentPrice){
  const cfg = slot.cfg;
  if ((cfg.super_trend_strictness_mode || 'manual') !== 'automatic') return true;
  if (!cfg.super_trend_auto_direction_gate_enabled) return true;
  if (slot.superCascadeLevel !== 'low') return true; // only relevant once loosened
  const ref = getSuperCascadeReference(slot);
  if (!ref || currentPrice == null || isNaN(currentPrice)) return true; // nothing open right now — no constraint
  const pass = ref.direction === 'sell' ? currentPrice < ref.price : currentPrice > ref.price;
  if (!pass) slot.emit(`Super auto-trend: directional gate — price ${currentPrice} not ${ref.direction === 'sell' ? 'below' : 'above'} oldest open trade's entry ${ref.price} — blocking`, 'err');
  return pass;
}
...
async function superFireTrade(slot, direction, firePrice){
  ...
  if (useRiseFall) {
    ...
    // Rotative Barrier (added) — resolved BEFORE any cascade bookkeeping
    // below, so an aborted/failed search never corrupts the fire-count or
    // rolling open-trades tracker with a trade that never actually fired.
    barrier = await resolveTradeBarrier(slot, direction === 'buy' ? '+' : '-', 'ONETOUCH');
    if (barrier == null) {
      slot.busy = false; // resolveTradeBarrier already logged why
      return;
    }
    label = direction === 'buy' ? 'BUY +Barrier' : 'SELL -Barrier';
  }
  slot.emit(useRiseFall ? `Super Logic signal: ${label}` : `Super Logic signal: ${label} ONETOUCH @${barrier}`);

  // Super cascade (added) — registers this fire regardless of outcome; a
  // no-op unless super_trend_strictness_mode is 'automatic'. Must happen
  // here (at the moment of firing), not on settlement, since loosening is
  // driven purely by how many trades have fired.
  registerSuperCascadeFire(slot, direction, firePrice);
  // Rolling directional reference (added) — this trade joins the open list
  // now (its gate check, if any, already happened in superOnCandle using the
  // list as it stood BEFORE this push). cascadeToken is popped off the list
  // the moment this specific trade settles, below.
  const cascadeToken = pushSuperCascadeOpenTrade(slot, direction, firePrice);
  ...
  if (concurrent) {
    ...
    runContract(slot, params, `SUPER ${label}`, { role:'primary', displayType:contractType, safeStakeController:ssCtrl })
      .then(rec => { popSuperCascadeOpenTrade(slot, cascadeToken); superAfterTradeSettled(slot, rec.outcome); })
      .catch(e  => { popSuperCascadeOpenTrade(slot, cascadeToken); slot.emit('Super trade error: '+e.message,'err'); superAfterTradeSettled(slot, 'loss'); });
  } else {
    ...
    runContract(slot, params, `SUPER ${label}`, { role:'primary', displayType:contractType })
      .then(rec  => { popSuperCascadeOpenTrade(slot, cascadeToken); superAfterTrade(slot, rec.won); })
      .catch(e   => { popSuperCascadeOpenTrade(slot, cascadeToken); slot.emit('Super trade error: '+e.message,'err'); superAfterTrade(slot, false); });
  }
}
...
function superOnCandle(slot, candle){
  ...
  // Daily Candle Color Filter (added) — direction is already 'buy'/'sell' here.
  if (!dailyCandleColorFilterOk(slot, direction)){
    slot.superRun = []; // discard all candles from this setup
    return;
  }
  ...
  // Super cascade directional gate (added) — only relevant in Automatic mode
  // once the cascade has loosened, and only if the toggle is on.
  if (!superCascadeDirectionOk(slot, parseFloat(candle.close))){
    slot.superRun = []; // discard all candles from this setup
    return;
  }
  ...
  slot.superRun = [];
  slot.superState = 'scanning';
  superFireTrade(slot, direction, parseFloat(candle.close)).catch(e => { slot.emit('Super fire error: '+e.message, 'err'); slot.busy = false; });
}
...
function superEfficiencyOk(slot){
  ...
  if (cfg.super_efficiency_mode !== 'rotating') {
    // Original single-threshold behavior — unchanged.
  ...
  // ── Rotating Efficiency (added) ──────────────────────────────────────────
  // Two thresholds, one active state per slot (starts at 'max'):
  //   • While active = 'max': need ratio >= rotate_max_ratio to pass. Passing
  //     means the market just proved it's trending seriously, so the bar
  //     LOOSENS to 'min' for the next check (ride the trend more easily).
  //   • While active = 'min': need ratio >= rotate_min_ratio to pass. Passing
  //     keeps the bar at 'min' (still trending seriously enough). FAILING
  //     means price may be reversing / going unserious, so the bar TIGHTENS
  //     back to 'max' for the next check.
  // Failing while active = 'max' just blocks the trade — already the
  // strictest setting, nothing to tighten further.
  const maxRatio = Number(cfg.super_efficiency_rotate_max_ratio) || 60;
  const minRatio = Number(cfg.super_efficiency_rotate_min_ratio) || 25;
  const activeLevel = slot.superEffRotateLevel === 'min' ? 'min' : 'max';
  const activeRatio = activeLevel === 'max' ? maxRatio : minRatio;
  const pass = ratio >= activeRatio;

  if (activeLevel === 'max' && pass) slot.superEffRotateLevel = 'min';       // trend confirmed — loosen
  if (activeLevel === 'min' && !pass) slot.superEffRotateLevel = 'max';     // trend fading — tighten back

  slot.emit(`Super: rotating efficiency — ${ratio.toFixed(1)}% net-vs-total over last ${lookback} ${tf.toUpperCase()} candles (active: ${activeLevel.toUpperCase()} @ ${activeRatio}%) — ${pass ? 'PASS' : 'FAIL'} — next check: ${slot.superEffRotateLevel.toUpperCase()}`, pass ? 'info' : 'err');
  return pass;
}
...
// ─── Daily Candle Color Filter (added) ─────────────────────────────────────
// Standalone — not part of the shared multi-timeframe filter family (always
// Daily/D1, no timeframe picker). Looks back over the last `lookback` fully
// closed D1 candles, counts how many are green vs red (a candle's own close
// vs its own open — not close vs previous candle, which is a different
// convention used elsewhere in this file), and requires at least `required`
// of them to match before allowing that direction to fire: green-majority
// for a + barrier (BUY), red-majority for a − barrier (SELL).
function dailyCandleColorFilterOk(slot, direction){ // direction: 'buy' or 'sell'
  const cfg = slot.cfg;
  if (!cfg.daily_candle_filter_enabled) return true;

  const lookback = Math.max(1, Number(cfg.daily_candle_filter_lookback) || 3);
  const required = Math.max(1, Number(cfg.daily_candle_filter_required) || 2);
  const buf = (slot.structD1Buf || []).slice(-lookback);
  if (buf.length < lookback) return true; // not enough D1 history yet — don't block startup
  ...
  const wantGreen = direction === 'buy';
  const count = wantGreen ? green : red;
  const pass = count >= required;
  if (!pass) slot.emit(`Daily Candle Filter: last ${lookback} D1 candles — ${green} green / ${red} red (need ${required} ${wantGreen ? 'green' : 'red'} for ${wantGreen ? '+' : '−'} barrier) — blocking`, 'err');
  return pass;
}
...
function startWatcher(slot){
  ...
  if (cfg.logic === 'super'){
    ...
    // Daily Candle Color Filter (added) — self-contained, gated only by its
    // own toggle (not part of structureNeedsTf's shared OR-chain).
    if (cfg.daily_candle_filter_enabled) {
      slot.candleSubs.push(candleManager.subscribe(cfg.symbol, 86400, candle => { if (!slot.running) return; slot.structD1Buf.push(candle); if (slot.structD1Buf.length>60) slot.structD1Buf.shift(); }));
    }
...
async function startSlot(slot){
  ...
  slot.structM30Buf=[]; slot.structH1Buf=[]; slot.structH4Buf=[]; slot.structD1Buf=[];
  ...
  slot.superCascadeLevel='high'; slot.superCascadeFireCount=0; slot.superCascadeLossStreak=0; slot.superCascadeOpenTrades=[];
  ...
  // Rotating Efficiency always restarts at 'max' — "bot starts with max efficiency".
  slot.superEffRotateLevel='max';
  superReset(slot);
...
function loadState(){
  try {
    ...
    data.forEach(d => {
      ...
      // One-time dedicated-market migration (added) — persisted cfg above
      // just overwrote symbol with whatever was saved before. Since this is
      // an EXISTING bot with its own saved state.json, defaultCfg(idx)'s new
      // per-slot market never even runs for it. This forces the dedicated
      // market onto this slot exactly once (marked so it never fires
      // again), while leaving it fully changeable afterward via the Market
      // dropdown same as always.
      if (!s.cfg.__slot_market_migrated) {
        s.cfg.symbol = SLOT_DEFAULT_MARKETS[s.idx % SLOT_DEFAULT_MARKETS.length];
        s.cfg.__slot_market_migrated = true;
      }
    });
...
app.get('/api/symbols', async (req, res) => {
  try {
    if (!symsCache) {
      ...
      const filtered = raw.filter(s => s.market==='synthetic_index' && (/^1HZ/.test(s.symbol) || /\(1s\)/i.test(s.display_name)))
        .map(s => ({ symbol:s.symbol, display_name:s.display_name }))
        .sort((a,b) => a.display_name.localeCompare(b.display_name));
      // Fixed (added): a transient/incomplete response from Deriv used to get
      // cached FOREVER (until server restart), silently locking the live/demo
      // Market dropdown to whatever few symbols happened to come back on that
      // one fetch. Now: only cache once we've actually seen every symbol we
      // know Deriv offers (DATAENGINE_MARKETS) — otherwise leave symsCache
      // unset so the NEXT request tries fetchSymbols() again fresh instead of
      // being stuck with a bad result.
      const gotAllKnown = DATAENGINE_MARKETS.every(sym => filtered.some(f => f.symbol === sym));
      if (gotAllKnown) symsCache = filtered;
      else { res.json(filtered); return; } // still answer this request, just don't cache the incomplete result
    }
    res.json(symsCache);
  } catch (e) { res.status(500).json({ error:e.message }); }
});
